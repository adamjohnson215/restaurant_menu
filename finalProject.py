...
def editRestaurant(restaurant_id):
	...
	if request.method == 'POST':
		if request.form['name']:
			...
			session.commit()
			flash(editedRestaurant.name +' Has Been Successfully Updated')
			return redirect(url_for('showRestaurants'))
	else:
...
def deleteRestaurant(restaurant_id):
	restaurantToBeDeleted = session.query(Restaurant).filter_by(id = restaurant_id).one()
	if request.method == 'POST':
		session.delete(restaurantToBeDeleted)
		session.commit
		flash(restaurantToBeDeleted.name + ' Has Been Successfully Removed')
		return redirect(url_for('showRestaurants'))
	else:
		return render_template('deleterestaurant.html', restaurant_id = restaurant_id, restaurant = restaurantToBeDeleted)
...
def editMenuItem(restaurant_id, menu_id):
	...
	if request.method == 'POST':
		if request.form['name']:
			...
			session.commit()
			flash(editedMenuItem.name +' Has Been Successfully Updated')
			return redirect(url_for('showMenu', restaurant_id = restaurant_id))
	else:
...
def deleteMenuItem(restaurant_id, menu_id):
	...
	if request.method == 'POST':
		...
		session.commit
		flash(itemToBeDeleted.name + ' Has Been Successfully Removed')
		return redirect(url_for('showMenu', restaurant_id = restaurant_id))
	else:
		return render_template('deletemenuitem.html', restaurant_id = restaurant_id, menu_id = menu_id, item = itemToBeDeleted)
